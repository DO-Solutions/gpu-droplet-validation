...
| `nvidia-fabricmanager fabric state == Completed on all GPUs` | On an NVSwitch SKU (`REQUIRES_NVSWITCH_FABRIC=1` in `nvidia_models.sh`), every GPU reports `fabric.state` of `Completed` within 60 s. The check polls every 5 s so a fabricmanager still initializing at suite start is waited out rather than failed. Non-NVSwitch SKUs skip the check and pass. | `nvidia-fabricmanager` is not running, or started and never finished. CUDA will not initialize on any GPU, so every collective downstream fails with `system not yet initialized`. **`In Progress` is a failure, not a pass**: a host whose fabricmanager died stays in that state forever, and treating it as a pass let broken hosts through to NCCL 30 minutes later. The diagnostic reports `states_seen` and `waited_seconds`. |
...
| Test | Threshold / criterion | What `not ok` means |
|---|---|---|
| `NCCL allreduce mean busbw@8GB >= 810 GB/s` | The mean of the per-run in-place bus bandwidths at the 8 GiB message size, across the 3 runs, is at least **810 GB/s** (and at least one run produced a busbw@8GB row at all) | This is the headline B300 collective-bandwidth floor. If no run produced a row, the collective could not complete — a hang, an abort, or an uninitialized NVSwitch fabric — and the point fails with `mean_busbw_8g_GBps: 0`. Below it means the GPU complex is not delivering the bandwidth a customer paying for a B300 expects — most often a fabric, NVLink, or PCIe regression. The diagnostic block includes `mean_busbw_8g_GBps`, `floor_GBps`, and `per_run_busbw_8g_GBps`. For per-size triage, the raw output of all three runs is saved to `results/nccl-allreduce_run1.log` … `_run3.log`. |
